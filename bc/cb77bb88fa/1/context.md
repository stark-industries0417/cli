...
Now with all this context I want to have better integration tests based on the flows we have in the docs. The integration tests we have now are abstracting away the actual agents which is fine for general testing but for example before a release or for larger refactors I want full tests from the outside. 

We already have a scripts/test-attribution-e2e.sh but I don't think doing it in bash is a good idea, can we do it in go?

- setup a tmp repo
- build a binary out of the current repo state
- en...
